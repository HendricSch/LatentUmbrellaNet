 """Hilfsskript zum Erzeugen eines Zarr-Stores mit latenten Darstellungen.

 Lädt ERA5-Batches, normalisiert sie, führt den VAE-Encoder
 aus und speichert die resultierenden latenten Darstellungen in einem Zarr-Store.
 """
...
def normalizer(x: torch.Tensor) -> torch.Tensor:
    """Kanalweise Normalisierung gemäß vorab berechneter Werte."""
    mean = np.array(
        [
            2.76707305e02,
            -1.02550652e-01,
...
@torch.no_grad()
def main():
    """Erzeugt den Zarr-Store `LATENT_STORE_PATH` mit latenten Darstellungen."""
    # Konstanten

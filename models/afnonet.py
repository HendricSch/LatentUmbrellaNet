 """AFNO (Adaptive Fourier Neural Operator) Komponenten.

 Referenzimplementierung angelehnt an NVlabs AFNO-Transformer. Dieses Modul
 enthält Bausteine, die in `AFNONet` zu einem Bild-zu-Bild-Vorhersagemodell kombiniert werden.
 """
...
class Mlp(nn.Module):
    """Zweischichtiges MLP mit GELU und Dropout."""
...
class Block(nn.Module):
    """AFNO-Block: Norm -> AFNO2D -> (optional) Double-Skip -> Norm -> MLP -> DropPath."""
    def __init__(
        self,
        dim,
        mlp_ratio=4.,
...
class AFNONet(nn.Module):
    """AFNO-basiertes Encoder/Decoder-Netz für Bild-zu-Bild Vorhersagen.

    Parameter wie `img_size`, `patch_size` und `embed_dim` folgen gängigen
    Vision-Transformer-Konventionen. `forward` gibt ein Bild in gleicher
    räumlicher Auflösung wie der Eingang zurück.
    """
    def __init__(
        self,
        img_size=(720, 1440),
...
class PatchEmbed(nn.Module):
    """Patch-Embedding via 2D-Convolution mit Stride = Patchgröße."""

 """Latent Umbrella Net (LUN).

 Dieses Modul bündelt einen vortrainierten Autoencoder (VAE, VQ-VAE) und ein
 Vorhersagenetz (UNet oder AFNO), um Wetterdaten im latenten Raum zu
 prognostizieren und anschließend wieder in den Pixelraum zu dekodieren.

 Kernidee:
 - Eingaben werden normalisiert und über den VAE-Encoder in Mittelwert
   und Log-Varianz abgebildet.
 - Ein Vorhersagenetz sagt die nächsten Zeitschritte voraus (autoregressiv über
   mehrere Zeitschritte, falls gewünscht).
 - Die vorhergesagten Zeitschritte werden mit dem VAE-Decoder in den 
   Pixelraum zurücktransformiert.
 """
...
class LatentUmbrellaNet(nn.Module):
    """Kombiniertes Modell aus VAE und Vorhersagenetz.

    Parameter
    ---------
    vae_ckpt_path:
        Pfad zu einem gespeicherten VAE-Checkpoint.
    vae_config_path:
        Pfad zur VAE-Konfigurationsdatei (YAML).
    prediction_net_type:
        Typ des Vorhersagenetzes: "unet" oder "afno".
    prediction_net_ckpt_path:
        Pfad zu einem gespeicherten Checkpoint des Vorhersagenetzes.
    device:
        Device-String (z. B. "cuda" oder "cpu").
    """
    def __init__(
    ...
    def forward(self, x_1: torch.Tensor, x_2: torch.Tensor, forecast_steps: int = 1) -> torch.Tensor:
        """Berechnet eine oder mehrere Vorhersagen und dekodiert sie in den Pixelraum.

        Parameter
        ---------
        x_1, x_2:
            Zwei aufeinanderfolgende Zeitpunkte im Datenraum, Form `(1, 69, 1440, 721)`
            oder bereits zugeschnitten `(1, 69, 1440, 720)`. Die Methode schneidet ggf.
            die letzte Breite automatisch ab, um 720 Latituden zu erhalten.
        forecast_steps:
            Anzahl autoregressiver Vorwärtsschritte im latenten Raum.

        Rückgabe
        --------
        torch.Tensor
        Letzte Vorhersage im Datenraum (d. h. Vorhersage zum Schritt
        `forecast_steps`). Form: `(1, 69, 1440, 720)`.
        """

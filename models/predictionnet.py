 """Vorhersagenetze für latente Momente.

 Dieses Modul stellt zwei Varianten für die Vorhersagenetzwerke im latenten Raum bereit:
 - UNet-basierte Architektur (`PredictionModel`), und
 - AFNO-basierte Architektur (`AFNOPredictionModel`).

 Hilfsklassen (ResBlock, AttentionBlock, Down/Up-Sampling usw.) definieren die
 Bausteine des UNet.
 """
...
class AFNOPredictionModel(pl.LightningModule):
    """Lightning-Modell, das AFNO als Vorhersagekern nutzt.

    Erwartet Eingaben der Form `(batch, 256, 180, 90)` (zwei Zeitpunkte mit je
    128 Kanälen). Padding/Unpadding stellt Größenkompatibilität mit AFNO sicher.
    """
    def __init__(self):
        super().__init__()
...
class PredictionModel(pl.LightningModule):
    """Lightning-Modell mit UNet als Vorhersagenetzwerk im latenten Raum.

    Erwartet Eingaben der Form `(batch, 256, 180, 90)`, wobei je 128 Kanäle
    zwei aufeinanderfolgende Zeitpunkte repräsentieren.
    """
    def __init__(self):
        super().__init__()
...
class UpSample(torch.nn.Module):
    """Nearest-Neighbor-Upsampling um Faktor 2."""
    def __init__(self):
...
class DownSample(torch.nn.Module):
    """Average-Pooling-Downsampling um Faktor 2."""
    def __init__(self):
...
class QKVAttention(torch.nn.Module):
    """Self-Attention auf Basis vordefinierter QKV-Faltung."""
...
class AttentionBlock(torch.nn.Module):
    """1D-Attention-Block über gefaltete räumliche Dimensionen."""
...
class DownBlock(torch.nn.Module):
    """Mehrere ResBlocks (optional mit Attention) und Downsampling."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class UpBlock(torch.nn.Module):
    """Upsampling, Skip-Connection und mehrere ResBlocks (optional mit Attention)."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class MidBlock(torch.nn.Module):
    """Zentrale UNet-Stufe mit zwei ResBlocks und optionaler Attention."""
...
class UNet(torch.nn.Module):
    """UNet für die Vorhersage im latenten Raum.

    Parameter
    ---------
    in_channels:
        Anzahl Kanäle pro Zeitpunkt (z. B. 128).
    base_channels:
        Basiskanäle der ersten Stufe.
    channels_mult:
        Multiplikatoren pro Stufe (bestimmt Kanalanzahlen der Stufen).
    num_resblocks:
        Anzahl ResBlocks pro Stufe.
    attention_layers:
        Ob pro Stufe Attention eingesetzt wird.
    pads:
        Padding (left, right, top, bottom) für Größenkompatibilität.
    """
    def __init__(
        self,
        *,
